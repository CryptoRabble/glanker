...
export async function getRedisClient2() {
  if (!redisClient) {
    console.log('Creating new Redis client');
    const redisUrl = process.env.REDIS_URL_2?.replace(/['"]/g, '');
...
export async function safeRedisGet2(key) {
  try {
...
export async function safeRedisSet2(key, value) {
  try {
...
export async function safeRedisDel2(key) {
  try {
